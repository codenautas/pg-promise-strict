...
describe('pg-promise-strict common tests', function(){
    ...
    var clientInternal = {mockClient: 'example of connection', query:function(){ throw new Error('you must mock this!');}};
    ...
    describe('internal controls', function(){
        ...
        before(function(done){
            ...
            pg.connect(connectParams).then(function(returnedConnection){
        ...
        it('control the parameters of the execute function',function(done){
            ...
            var clientInternalControl = expectCalled.control(connection.internals.connection,'query',{returns:[
                queryInternal
            ]});
            connection.query().execute('one value', 'other value').then(function(result){
        ...
        it('control the log',function(done){
            ...
            Promises.start(function(){
                return connection.query('select $1, $2, $3, $4', [1, "one's", true, null]).execute();
                ...
                messages=[];
                return connection.query("select 'exit'").execute();
            }).then(function(result){
                expect(messages).to.eql([
                    '------',
                    "select 'exit';",
